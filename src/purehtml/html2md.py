...
def convert_li_element(li, level=-1, idx=0):
...
def convert_list_element(element, level=-1):
    level += 1
    for idx, li in enumerate(element.find_all("li")):
        new_li = convert_li_element(li, idx=idx, level=level)
        for child in new_li.find_all(LIST_TAGS, recursive=False):
            convert_list_element(child, level=level)
